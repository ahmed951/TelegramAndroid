    ...
    private Runnable recordAudioVideoRunnable = new Runnable() {
        ...
        public void run() {
            ...
            if (isInVideoMode()) {
                ...
                MediaController.getInstance().startRecording(currentAccount, dialog_id, replyingMessageObject, getThreadMessage(), storyItem, replyingQuote, recordingGuid, true, parentFragment != null ? parentFragment.quickReplyShortcut : null, parentFragment != null ? parentFragment.getQuickReplyId() : 0, getSendMonoForumPeerId(), getSendMessageSuggestionParams());
                recordingAudioVideo = true;
                updateRecordInterface(RECORD_STATE_ENTER, true);
                if (recordTimerView != null) {
...
//        });
    ...
    private void showAiButton(boolean show_) {
        final boolean show = (show_ || richDraftActive) && parentFragment != null && !parentFragment.isSecretChat() && !MessagesController.getGlobalMainSettings().getBoolean("hideAiEditor", false);
...
=======
                SendMessagesHelper.SendMessageParams params = SendMessagesHelper.SendMessageParams.of(textMessageString, dialog_id, replyingMessageObject, replyToTopMsg, messageWebPage, messageWebPageSearch, entities, null, null, notify, scheduleDate, scheduleRepeatPeriod, sendAnimationData, updateStickersOrder);
>>>>>>> upstream/dev
                params.quick_reply_shortcut = parentFragment != null ? parentFragment.quickReplyShortcut : null;
                params.quick_reply_shortcut_id = parentFragment != null ? parentFragment.getQuickReplyId() : 0;
                params.effect_id = effectId;
...
//        }
    ...
    public void setVoiceDraft(MediaDataController.DraftVoice draft) {
        ...
        TL_stories.StoryItem storyItem = delegate != null ? delegate.getReplyToStory() : null;
        MediaController.getInstance().prepareResumedRecording(currentAccount, draft, dialog_id, replyingMessageObject, getThreadMessage(), storyItem, replyingQuote, recordingGuid, parentFragment != null ? parentFragment.quickReplyShortcut : null, parentFragment != null ? parentFragment.getQuickReplyId() : 0, getSendMonoForumPeerId(), getSendMessageSuggestionParams());
    }
